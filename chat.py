...
logger = logging.getLogger('zylo.chat')
...
# ---------------------------
# Logging Utilities for SocketIO
# ---------------------------
def log_socket_event(event_name, data, user_id=None):
    from flask import request
    uid = user_id or data.get('user_id', 'UNKNOWN')
    logger.info(f"SOCKET EVENT | {event_name} | User: {uid} | IP: {request.remote_addr} | Data: {json.dumps(data)}")
...
def on_join(data):
    room_id = data['room_id']
    user_id = data['user_id']
    log_socket_event('join_room', data)
...
def on_send(data):
    ...
    log_socket_event('send_message', {'room_id': room_id, 'sender_id': sender_id, 'type': msg_type, 'filename': fname})
